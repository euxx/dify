...
在开始高级接入之前，请确保你已经阅读过[快速接入](./tool_scale_out.md)，并对AI Mate的工具接入流程有了基本的了解。
...
AI Mate支持`文本` `链接` `图片` `文件BLOB` `JSON` 等多种消息类型，你可以通过以下几个接口返回不同类型的消息给LLM和用户。

注意，在下面的接口中的部分参数将在后面的章节中介绍。

#### 图片URL
只需要传递图片的URL即可，AI Mate会自动下载图片并返回给用户。
...
- `meta` 文件的元数据，如果你知道该文件的类型，最好传递一个`mime_type`，否则AI Mate将使用`octet/stream`作为默认类型
...
该工具需要传入user_id和需要进行总结的文本，返回一个总结后的文本，AI Mate会使用当前工作空间的默认模型对长文本进行总结。
...
该工具需要传入需要爬取的网页链接和一个user_agent（可为空），返回一个包含该网页信息的字符串，其中`user_agent`是可选参数，可以用来识别工具，如果不传递，AI Mate将使用默认的`user_agent`。
