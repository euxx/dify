...
Before starting with this advanced guide, please make sure you have a basic understanding of the tool integration process in AI Mate. Check out [Quick Integration](./tool_scale_out.md) for a quick runthrough.
...
AI Mate supports various message types such as `text`, `link`, `json`, `image`, and `file BLOB`. You can return different types of messages to the LLM and users through the following interfaces.

Please note, some parameters in the following interfaces will be introduced in later sections.

#### Image URL
You only need to pass the URL of the image, and AI Mate will automatically download the image and return it to the user.
...
- `meta` The metadata of the file, if you know the type of the file, it is best to pass a `mime_type`, otherwise AI Mate will use `octet/stream` as the default type
...
This tool takes in an user_id and the text to be summarized, and returns the summarized text. AI Mate will use the default model of the current workspace to summarize the long text.
...
This tool takes in web page link to be crawled and a user_agent (which can be empty), and returns a string containing the information of the web page. The `user_agent` is an optional parameter that can be used to identify the tool. If not passed, AI Mate will use the default `user_agent`.
